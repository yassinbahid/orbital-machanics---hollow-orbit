function [d,TE] = shootingF_OC2H(X)
...
%        legend('V_0','V(t_f) Attained', 'V_f Target')
        ...
        thrust_mag = .5*(M(:,8).^2+M(:,7).^2).^.5/k2;
%         cut = 0;
%         ibn2=find(pheta>cut);
%         pheta(ibn2) = pheta(ibn2)-2*pi;
        figure
        subplot(2,1,1)
        title('control plot (\rho, \theta)')
        plot(T,pheta,'g')
        ...
        plot(T,thrust_mag,'r')
        xlabel('Time (1 = 4.3 days)')
        ylabel('Thrust Magnitude (1 = )')
        hold on
        
        % Control Vectors
        figure
        plot(M(:,1),M(:,2), 'r')       % trajectory.
        hold
        quiver(M(1:5:end,1),M(1:5:end,2),-M(1:5:end,7),-M(1:5:end,8))
...
%       cth = -lam3/l34n;
%       sth = -lam4/l34n;
% Terms for OCP4 (objective with int k1(x^2+y^2) + k2U^2 dt
 %k1 = .99;, k2=.01;S5_term = -2*k1*(x-LN(1));,S6_term = -2*k1*y; % OCP4
 global K1 K2
 k1=K1;,k2=K2;
% k1 = 0;, k2=1;
 S5_term = -2*k1*(x-LN(1));,S6_term = -2*k1*y; % OCP3
...
% Formulation of State ODEs for x, y xdot, ydot:
      ...
      S_dot(1) = vx;
      S_dot(2) = vy;
      S_dot(3) = 2*vy + x - um1or13*xpu - uor23*xpum1 + U3;  %Thrust*cth;
      S_dot(4) =-2*vx + y - y*(um1or13   + uor23)     + U4;  %Thrust*sth;
...
% Adjoint ODEs:   dlam = -DF^T lam
      ...
      S_dot(5) =                - F31*lam3 - F41*lam4 + S5_term;
      S_dot(6) =                - F32*lam3 - F42*lam4+  S6_term;
      S_dot(7) = -lam1                     +   2*lam4;
      S_dot(8) =        -lam2   - 2*lam3;
